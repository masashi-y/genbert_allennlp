...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if random_shift:
            shift = torch.randint(
                512 - seq_length + 1, size=(input_shape[0], 1), device=device
            )
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, attention_mask, head_mask=None):
        ...
        context_layer = context_layer.permute(
            0, 2, 1, 3
        ).contiguous()  # [b, query_seq_len, n_heads, head_size]
        new_context_layer_shape = context_layer.size()[:-2] + (
            self.all_head_size,
        )  # [b, query_seq_len, hidden_size]
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_output = self.self(
            input_tensor, input_tensor, input_tensor, attention_mask, head_mask
        )
        if self.output_attentions:
        ...
        if self.output_attentions:
            return (
                attentions,
                attention_output,
            )  # attentions are that of self attn and not src attn
        return attention_output
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.attention(
            hidden_states, attention_mask, head_mask, memory_tensor, src_attention_mask
        )
        if self.output_attentions:
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        def extend(mask, is_decoder=False):
            extended_mask = (
                mask.unsqueeze(1).unsqueeze(2).to(dtype=torch.float)
            )  # [bsz, 1, 1, key_seq_len]
            ...
            if is_decoder:
                ...
                subsequent_mask = torch.triu(torch.ones_like(extended_mask)).transpose(
                    2, 3
                )
        ...
        extended_attention_mask = extend(
            attention_mask, memory_tensor is not None)
        extended_src_attention_mask = (
            None if src_attention_mask is None else extend(src_attention_mask)
        )
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.bert(
            input_ids, token_type_ids, attention_mask, random_shift=random_shift
        )
...
class SimpleMLMHead(nn.Module):
    ...
    def __init__(self, embed):
        ...
        self.decoder = nn.Linear(embed.weight.size(
            1), embed.weight.size(0), bias=False)
    ...
    def forward(
        ...
        prediction_scores = (
            self.decoder(sequence_output) + self.bias
        )  # [bsz, seq_len, vocab]
        preds = prediction_scores.argmax(dim=-1)  # [bsz, seq_len]
        if labels is None:
            return preds
        ...
        loss_fct = CrossEntropyLoss(
            ignore_index=ignore_idx, reduction="none" if sample_wise else "mean"
        )
        ...
        if not sample_wise:
            ...
            element_wise_nll = nlls.view(
                *labels.size())  # 0.0 for ignored indices
            sample_wise_nll = element_wise_nll.sum(dim=-1) * include.float()
            # , errors                    # [bsz], [bsz]
            return sample_wise_nll
...
class BertTransformer(BertPreTrainedModel):
    ...
    def encode(
        ...
        outputs = self.bert(
            input_ids,
            token_type_ids,
            self.mask(input_ids, input_mask),
            random_shift=random_shift,
            inputs_embeds=inputs_embeds,
        )
        sequence_output, _ = outputs
    ...
    def forward(
        ...
        if task.lower() == "mlm":
            return self.mlm_task(
                input_ids, None, input_mask, target_ids, ignore_idx=ignore_idx
            )
        ...
        encoder_output, pooled_output = self.encode(
            input_ids=input_ids,
            token_type_ids=None,
            input_mask=input_mask,
            random_shift=random_shift,
        ...
        dec_out, _ = self.bert(
            target_ids_in,
            attention_mask=self.mask(target_ids_in),
        ...
        dec_nlls = self.dec_head(
            self.extra_dec_layer(dec_out),
            target_ids_out,
            ignore_idx=0,
            sample_wise=True,
        )
    ...
    def inference(
        ...
        start_ids = torch.full_like(
            input_mask[:, 0], fill_value=start_tok_id, dtype=torch.long
        )
        dec_out_ids = (
        ...
        for _ in range(
            ...
            dec_out, _ = self.bert(
                dec_out_ids,
                attention_mask=self.mask(dec_out_ids),
                memory_tensor=encoder_output,
                src_attention_mask=input_mask,
            )
...
class SpanExtractionHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self._question_span_start_predictor = self.ff(
            2 * bert_dim, bert_dim, 1)
    ...
    def forward(
        ...
        passage_mask, question_mask = (
            token_type_ids.bool(),
            ((1 - token_type_ids) * input_mask.long()).bool(),
        )
        ...
        question_vector = self.summary_vector(
            passage_out, question_mask, "question")
        ...
        best_answer_ability = torch.argmax(
            answer_ability_log_probs, 1).unsqueeze(1)
        ...
        span_preds = (
            best_passage_span.float() * (1 - best_answer_ability.float())
            + best_question_span.float() * best_answer_ability.float()
        ).long()
        ...
        log_marginal_likelihood_for_passage_span = self._span_log_likelihood(
            answer_as_passage_spans,
            passage_span_start_log_probs,
            passage_span_end_log_probs,
        )
        ...
        log_marginal_likelihood_for_question_span = self._span_log_likelihood(
            answer_as_question_spans,
            question_span_start_log_probs,
            question_span_end_log_probs,
        )
        ...
        return marginal_log_likelihood, loss, start_preds, end_preds
    ...
    def _passage_span_module(self, passage_out, passage_mask):
        ...
        # Shape: (batch_size, seq_length)
        passage_span_end_logits = self._passage_span_end_predictor(passage_out).squeeze(
            -1
        )
    ...
    def _question_span_module(self, passage_vector, question_out, question_mask):
        ...
        encoded_question_for_span_prediction = torch.cat(
            [
                question_out,
                passage_vector.unsqueeze(1).repeat(1, question_out.size(1), 1),
            ],
            -1,
        )
    ...
    def _span_log_likelihood(
        ...
        gold_span_mask = gold_span_starts != -1
        ...
        log_likelihood_for_span_starts = torch.gather(
            span_start_log_probs, 1, clamped_gold_span_starts
        )
        log_likelihood_for_span_ends = torch.gather(
            span_end_log_probs, 1, clamped_gold_span_ends
        )
        # Shape: (batch_size, # of answer spans)
        ...
        log_likelihood_for_spans = util.replace_masked_values(
            log_likelihood_for_spans, gold_span_mask, -1e7
        )
        # Shape: (batch_size, )
